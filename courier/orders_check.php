<div class="menu">
    <?php 
        ...
        $order_query = "select numb_order, sum(price), status, title_status, comment, date, employee FROM orders inner join status where orders.status = status.id_status and status = 2 and employee = $id_cur group by numb_order, status, title_status, comment, date, employee";
        ...
        <div class="menu_content">
            ...
            <div class="card tovar_card">
                ...
                <div class="card_text tovar_text">
                    ...
                    <?php if ($order['title_status'] == "Подтвержден") {
                    ?>

                    <a href="?del=<?=$order['numb_order'];?>">Отклонить заказ</a>

                    <a href="?ap=<?=$order['numb_order'];?>">Принять заказ</a>
                    <?php
                    }
                    ?>
...
if ($_GET['ap']) {
    ...
    if ($del_result && $cur_result) {
        echo "<script>alert('Заказ принят!!');location.href='ac_orders_check.php';</script>";
    }
    else{
